<div id="user-import-modal" class="w-full h-full fixed top-0 left-0 z-50 transition-all duration-500 hidden overflow-y-auto">
    <div class="sm:max-w-md sm:w-full m-3 sm:mx-auto flex flex-col bg-white shadow-sm rounded dark:bg-gray-800">
        <form id="import-form" method="POST" action="{{ route('admin.data-warga.import-excel') }}" enctype="multipart/form-data">
            @csrf

            <div class="flex justify-between items-center py-2.5 px-4 border-b dark:border-gray-700">
                <h3 class="font-medium text-gray-600 dark:text-white text-lg">
                    Form Import Data Warga RT.36
                </h3>
            ...
            <div class="p-4 space-y-4 text-sm">
                <div class="mb-3">
                    <p class="text-sm text-gray-600">  
                        <a href="{{ route('admin.data-warga.template-excel') }}" 
                ...
                <div class="mb-3">
                    <label class="mb-2" for="file_excel_warga">Upload File</label>
                    <input type="file" name="file_excel_warga" id="file_excel_warga" class="form-input border">
                </div>
            </div>
